"""Bộ kiểm thử đối soát tính toàn vẹn và bất biến dữ liệu.

Bảo đảm các định luật bảo toàn dữ liệu xuyên suốt các tầng Medallion Lakehouse:
1. Bất biến doanh thu: SUM(Silver.Revenue) == SUM(FactSales.Revenue) == Mart Overview Total_Revenue.
2. Grain ổn định: Mỗi khóa FactSales đại diện đúng 1 line item trong 1 order.
3. Bảo toàn số dòng: Raw = Clean + Quarantine + Deduplicated.
"""
...
def test_gold_revenue_reconciles_with_silver_and_marts(spark_session):
    ...
    # 3. Doanh thu từ Mart Overview.
    overview_revenue = round(marts["mart_overview"].select("Total_Revenue").collect()[0][0], 2)

    assert silver_revenue == 1422.0
    assert fact_revenue == silver_revenue, (
        f"Fact Revenue ({fact_revenue}) lệch với Silver ({silver_revenue})"
    )
    assert overview_revenue == fact_revenue, (
        f"Overview Mart Revenue ({overview_revenue}) lệch với Fact ({fact_revenue})"
    )
